...
class ReinforcementLearner:
	def __init__(self, state_size, action_size, learn_rate=0.03):
		...
		self.model.add(Dense(input_dim=state_size, output_dim=50, activation="tanh"))
...
class LethalInterface:
	...
	def playOneFrame(self):
		...
		if self.last_update_time + 10 > nowTicks:
			return
	...
	def _do_learning(self, forceReward = 0):
		...
		if self.previous_action != None:
			reward = forceReward if forceReward != 0 else self._get_reward()

			# Actual Q = Current max Q + reward
			self.previous_qs[0][self.previous_action] = best_action_q + reward
	...
	def _get_state(self):
		...
		# Player 0 charge timer
			...
			correctedBallSpeed,
			ord(game_data.ball_state.direction),
